...
''' This model applies the method described in http://cnnlocalization.csail.mit.edu/Zhou_Learning_Deep_Features_CVPR_2016_paper.pdf to learn GlobalAveragePooling weights in order to visualize the areas where the discriminator poses more attention '''
...
def build_critic(mri_shape, seg_shape):
    ...
    pred = layers.Dense(1, activation='sigmoid', use_bias=False, kernel_initializer='uniform')(gap)
    #out = tf.stack([enc1fl, tf.tile(enc3fl, [1,4])], axis=-1)
    return Model([c_mri_in, c_seg_in], [out, pred, gap], name='C')
