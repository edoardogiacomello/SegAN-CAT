...
class DeepMRI():
    ...
    @tf.function
    def train_step(self, x, y, train_g=True, train_d=True, buffer=None):
        '''
        ...
        '''
        history = None # History will be (x, fake, true)
                
        if buffer is not None:
            history_ids = tf.random.shuffle(tf.range(self.buffer_size))[:self.batch_size]
            history = (tf.gather(buffer[0], history_ids), tf.gather(buffer[1], history_ids), tf.gather(buffer[2], history_ids))
            
        with tf.GradientTape(persistent=True) as tape:
            g_output = self.generator(x, training=train_g)
            
            if history is None:
                history = (x, g_output, y)
            
            h_real = self.discriminator([history[0], history[1]], training=train_d)
            h_fake = self.discriminator([history[0], history[2]], training=train_d)
            
            d_real = self.discriminator([x, y], training=train_d)
            d_fake = self.discriminator([x, g_output], training=train_d)
            
            h_loss = self.arch.loss_d(h_real, h_fake)
        ...
        if train_d == True:
            ...
            d_hist_gradients = tape.gradient(h_loss, self.discriminator.trainable_variables)
            self.d_optimizer.apply_gradients(zip(d_hist_gradients, self.discriminator.trainable_variables))
        ...
        if buffer is None:
            ...
            indices_to_include = tf.random.shuffle(tf.range(self.buffer_size))[:self.buffer_size//2]
            if buffer is None:
                buffer = (tf.concat([history[0],]*2, axis=0), tf.concat([history[1],]*2, axis=0), tf.concat([history[2],]*2, axis=0))
            else:
                buffer = (tf.concat([tf.random.shuffle(buffer[0])[:-self.buffer_size//2], tf.gather(x, indices_to_include)], axis=0), tf.concat([tf.random.shuffle(buffer[1])[:-self.buffer_size//2], tf.gather(g_output, indices_to_include)], axis=0), tf.concat([tf.random.shuffle(buffer[2])[:-self.buffer_size//2], tf.gather(y, indices_to_include)], axis=0))

        return [g_loss, d_loss], self.compute_metrics(y, g_output), buffer
    ...
    def train(self, alternating_steps=None, tracked_metric='dice_score', tracked_metric_maximize=True, save_every_epochs=20):
        ...
        for e in range(self.current_epoch, self.max_epochs):
            ...
            for i, row in enumerate(self.train_dataset()):
                ...
                train_losses, train_metrics, buffer = self.train_step(row['mri'], row['seg'], train_g=train_g, train_d=train_d, buffer=self.buffer)
                self.buffer = buffer
